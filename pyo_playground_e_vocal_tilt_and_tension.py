...
# USE pyo.Balance FOR BALANCING OUT AUDIO AMPLITUDES
F0 = 407
F1 = 538 # praat value; orig 610
F2 = 1779 # praat value; orig 1900
F3 = 2751 # praat value
SOURCE_TENSION = Sig(0)  # blending between the -6 dB/octave sloped filter and the -12 dB/octave one
...
true_F0 = F0 + fundamental_sway + FM_jitter
vocal = Blit(freq=true_F0, harms=80, mul=1 + vocal_amp_sway)
body = ButLP(vocal, 400, mul=1)  # spectral "tilt" (spectral shaping of Blit)
body_high = ButHP(vocal, 3000, mul=0)
vocal = body + body_high
f0 = ButLP(Reson(vocal, true_F0, 1, mul=0.6), (true_F0) * 1.5)  # this thing... it may work
...
noise = Noise()
f0noise = ButLP(Reson(noise, true_F0, 5, mul=1), (true_F0) * 1.5)
f1noise = Reson(noise, F1, 30, mul=0.5)
f2noise = Reson(noise, F2, 40, mul=0.2)
f3noise = Reson(noise, F3, 60, mul=0.025)
sum = (f0 + f1 + f2 + f3) * 30 + (f0noise + f1noise + f2noise + f3noise) * 0.05
sum.out(0)
sum2 = sum * 1
sum2.out(1)
